...
def option_two():
	...
	card_no = Entry(option2, width = 30)
	card_no.grid(row = 1, column = 1)
	
	card_no_label = Label(option2, text ='Card_no') 
	...
	submit_btn = Button(option2, text ='Add Borrower ', command = option_two_cur.execute)
	submit_btn.grid(row = 10, column =0, columnspan = 2, pady = 10, padx = 10, ipadx = 140)


	option_two_cur.execute("INSERT INTO BORROWERS VALUES (:card_no, :name, :phone, :address) ",
		{   'card_no': card_no.get(),
			'name': name.get(),
			'phone': phone.get(),
			'address': address.get(),
